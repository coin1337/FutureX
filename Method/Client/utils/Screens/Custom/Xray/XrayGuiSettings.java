...
public class XrayGuiSettings {
   ...
   public XrayGuiSettings(Block... blocks) {
      Stream var10000 = ((Stream)Arrays.stream(blocks).parallel()).map(XrayGuiSettings::getName).distinct().sorted();
   ...
   public static void fromJson(JsonElement json) {
      if (json.isJsonArray()) {
         ...
         }).filter(Objects::nonNull).map(XrayGuiSettings::getName).distinct().sorted().forEachOrdered((s) -> {
            blockNames.add(s);
         });
      }
   }
